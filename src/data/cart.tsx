...
const Cart = () => {
  ...
  const handleBlur = (productId: number, value: string) => {
    const parsedValue = parseInt(value, 10); // Changed from isNaN(value)
                        ...
                        <div className="mt-4 flex justify-between items-center">
                          ...
                          <div className="flex items-center border border-gray-300 rounded">
                            ...
                            <input
                              ...
                              style={{ pointerEvents: 'auto' }} // Added style for interactivity
                              value={itemInputQuantities[item.product.id] || ''} // Used || '' instead of ?? ''
                              onChange={(e) => handleInputChange(item.product.id, e.target.value)}
                              onBlur={(e) => handleBlur(item.product.id, e.target.value)} // Corrected onBlur call
                            />
                            <button
                              onClick={() => handleQuantityChange(item.product.id, item.quantity + 1)}
                              className="p-1 hover:bg-gray-100"
