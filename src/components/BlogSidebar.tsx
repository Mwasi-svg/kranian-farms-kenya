          ...
          {recentPosts.map(post => (
            <div key={post.id} className="flex items-start">
              ...
              <div className="ml-3">
                <Link 
                  to={`/blog/${post.slug}`} 
                  className="text-sm font-medium hover:text-kranian-600 transition-colors line-clamp-2 story-link"
                >
                  {post.title}
                </Link>
          ...
          {categories.map(category => (
            <div key={category.name} className="flex justify-between items-center">
              <Link 
                ...
                className="text-gray-700 hover:text-kranian-600 transition-colors story-link"
              >
                {category.name}
              </Link>
